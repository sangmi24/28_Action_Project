...
<body>
 ...
 %>
  ...
  리스트의 길이 : ${ list.size() } <br> <!-- 자바 방식 -->
  <!-- fn:length 메소드는 문자열의 길이 뿐만 아니라 ArrayList의 사이즈도 알아낼 수 있는 메소드다.  -->
  
  모두 대문자로 출력 : ${ fn:toUpperCase(str) } <br>
  모두 소문자로 출력 : ${ fn:toLowerCase(str) } <br>  
  
 are 의 시작인덱스 : ${ fn: indexOf(str, 'are') }<br>
 ...
 str에 "are" 이라는 문자열이 포함되어 있니? :  ${fn:contains(str, "are") } <br>
