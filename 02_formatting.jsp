...
<body>
  ...
  <h1>1.formatNumber</h1>
   ...
   <p>
      ...
      [표현법]<br>
      &lt;fmt:formatNumber value="포맷을지정하고자하는값" groupingUsed="true/false(생략가능)" 
                                type="percent/currency(생략가능)" currencySymbol="$(생략가능)" /&gt;
       <br>                         
      => groupingUsed : true로 지정하면 3자리마다  ,로 구분짓는 역할(생략시 true가 기본값)<br>
      => type : percent(소숫점을 백분율로 변경해서)/ currency(통화기호를 붙여서 보여주는 역할, 자동으로 로컬단위가 붙음)<br>
   ...
   <!-- 
      type="currency" : 통화(돈) 단위로 보여짐
                        ...
                        + groupingUsed 속성이 기본값으로 true 이기 때문에 3자리마다 구분되어 있음  
    -->
   ...
   <!-- 
    ...
    <p>
     ...
     - 단, 무조건 날짜 형식(java.util.Date 객체)으로 value를 지정해야 한다.  
    </p>
  
    <!--테스트 데이터 셋팅 -->
    <c:set var="current" value="<%= new java.util.Date() %>" />
    <!-- 
       pageContext.setAttribute("current", new java.util.Date());
     -->
     그냥 출력 : ${ current } <br>
  ...
  <ul>
    <li>
      ...
      <!-- type="date" :날짜를 출력하겠다. type 속성 생략시 기본값은 "date"  -->   
    </li>
    <li>
