...
<body>
 ...
 수강생 정보:  
  ...
  <h3>2.EL 을 이용해서 보다 쉽게 해당 Scope 에 저장된 값들 출력하기</h3>
  
  <p>
     EL은 getXXX를 통해 값을 빼올 필요 없이 키값만 제시하면 바로 내가 원하는 값에 접근 가능 <br>
         내부적으로  해당 Scope 영역에 해당 키값의 밸류값을 가져올 수 있음 <br>
     
         기본적으로 EL 은 JSP 내장 객체를 구분하지 않고 자동적으로 모든 내장객체를
         범위가 작은 순으로 순차적으로 키값을 검색하여 존재하는 경우에만 값을 가져옴 <br>
  ...
  <p>
   ...
   <%--
      teacher 라는 키값으로 접근했을때 밸류값은 Person 객체임
          해당 Person객체의 각 필드의 담긴 값을 출력하고자 한다면,
      .필드명으로 접근하면 됨
            내부적으로 getter메소드를 찾아서 실행됨
      (즉, VO클래스를 만들 경우 getter메소드는 항상 만들어야 함)
    --%>
    <ul>
  ...
  <!-- 
      EL은 공유범위가 가장 작은 Scope 에서부터 해당 키값을 순차적으로 검색함
      pageScope => requestScope => sessionScope=> applicationScope
         순으로 속성을 찾음
         
         만약 모든 영역에서 해당 키값을 못찾는 경우에는
         아무것도 출력이 되지 않음(500오류 안남)
   -->
  ...
  %>
   ...
   requsetScope 에 담긴 값 : ${requestScope.scope } <br>
