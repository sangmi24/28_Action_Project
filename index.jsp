...
<body>
   ...
   <!-- 
     * JSP 를 이루는 구성인자
       1. JSP 스트립팅 원소 : JSP 페이지에서 자바코드를 직접 기술 할 수 있도록 하는 기술
                                           예) 선언문, 스크립틀릿, 표현식(출력식)
       2. 지시어 : JSP 페이지 정보에 대한 내용을 표현한다거나 또다른 페이지를 포함할때 쓰는 기술
                                   예) page 지시어, include 지시어, taglib 지시어(라이브러리 추가 시 사용)
       3. JSP 액션 태그 : XML 기술을 이용해서 기존의 jsp문법을 확장하는 기술을 제공하는 태그 
       - 표준 액션 태그 (Standard Action Tag) : JSP 페이지에서 바로 가용 가능 ( 별도의 라이브러리 연동이 필요없음)
                                                                                             표준 액션 태그는 모든 태그명 앞에  jsp: 이라는 접두어가 붙음
       - 커스텀 액션 태그 (Custom Action Tag) : JSP 페이지에서 바로 사용 불가능 (별도의 연동이 필요함)                           
