...
<body>
   <!-- taglib 지시어 누락 prefix, uri 속성 오타 시: Unknown Tag 오류 발생!! -->
 ...
 * 변수 선언(&lt;c:set var="변수명" value="리터럴" scope="스코프영역지정(생략가능)"&gt;)
 ...
 - 해당 변수를 어떤 scope에 담아둘건지 지정 가능함
  (생략 시 기본적으로 pageScope 에 담김)
  => 즉, 해당 scope영역에 setAttribute ("키", 밸류)메소드가 내부적으로 실행되는 원리
  ...
  * 주의사항
  ...
  - 반드시 해당 변수의 담아두고자하는 초기값 속성은 무조건 셋팅해야함
        (즉, 선언과 동시에 초기화) 
 </pre>
 ...
 <!--
  ...
  <!-- 
   requestScope 에 num2이라는 키값으로 20이라는 값이 담긴 상태
   ...
   num2 변수값 : ${ num2 } <br>
    ...
    <!-- 
        sessionScope 에 result 라는 키값으로 num1값과 num2값을 합친 결과물이 담긴 상태
        session.setAttribute("result",(int)pageContext.getAttribute("num1")+(int)request.getAttribute("num2")); 구문과 동일한 역할
     -->
      ...
      * 변수 삭제 (&lt;c:remove var="제거하고자하는변수명" scope="스코프영역지정(생략가능)"&gt;)
      ...
      - scope 지정 생략시 모든 scope에서 해당 변수를 찾아서 전부 삭제
      => 즉, 해당 scope에 .removeAttribute("키")라는 메소드를 이용해서 제거하는것
  ...
  request 에서 삭제 후 result : ${ result } <br><br>  <!-- 이 시점에서는  sessionScope 에 담긴 값이 찍힐 것임  -->
  <!-- 
     request.removeAttribute("result");
   -->
  
  
  2) 모든 scope 에서 삭제 (scope 속성 생략) <br> 
  <c:remove var="result" />  
   ...
   <pre>
   * 변수 출력(&lt;c:out value="출력하고자하는값" default="기본값(생략가능)" escapeXml="true(기본값)/false"&gt;)
    - 데이터를 출력하고자 할 때 사용하는 태그
    - default : value에  출력하고자 하는 값이 없을 경우 기본값으로 출력할 내용물을 기술(생략 가능)
    - escapeXml : 태그로써 해석해서 출력할지의 여부 (생략가능, 생략시 true 가 기본값)
   </pre>  
   ...
   <!-- escapeXml 생략시 기본값이 true=태그 해석 안됨 (문자열로 취급) -->
   ...
   <pre>
    -JAVA 의 단일 if 문과 비슷한 역할을 하는 태그
    -단, 조건식은 test 속성에 적되, EL구문으로 작성해야 함!! 
   </pre>
   
   <!-- 기존 방식 -->
   <% if(true) { %>
   
      <!-- 조건식이 true 일 경우 보여질 html 태그들 -->
   ...
   <c:if test="${num1 le num2 }"> <!-- 조건식이 true 이므로 해당 내용물이 보임 -->
   ...
   <% if(( (String)pageContext.getAttribute("str") ).equals("안녕하세요 ")) {%>
        <!-- str 키값에 담긴 내용물과 "안녕하세요"기 일치할 경우 보여질 내용물 -->
   <% } %>
   ...
   - 각 조건들을 c: choose의 하위 요소로 묶어서 c:when 으로 조건을 작성 
  ...
  => step : 기존의 일반 for문의 증감식에 해당되는 속성 (생략가능, 생략시 1이 기본값)
  
     향상된 for문(변수선언부, 배열명 또는 컬렉션명)
   ...
   <% } %>
   --%>
   <c:forEach var= "i" begin="1" end="10" > <!-- step속성 생략시 기본값 1 -->
        반복 확인 : ${ i } <br>
    </c:forEach>
   ...
   <%-- 
   <% for(Board b : list) {%>
      <!-- b.getter메소드 이용해서 매번 테이블을 만들거나 했었음 -->
   <% } %>
   --%>
   ...
   <!-- 응용 : DB로 부터 위의 내용들을 조회했다라는 가정하에 table로 구성하기 -->
   <table border="1">
       <thead>
          <tr>
            <th>순번</th>
       ...
       <tbody>
           <!-- 기존방법: if문으로 비어있나 안비어있나 검사 후 안비어있다면 그때 반복  -->
           <%-- 
	           <% if(pList.isEmpty()) {%>
   ...
   <pre>
    - JAVA 의 StringTokenizer 클래스, split("구분자"); 와 유사한 역할
    - 구분자를 통해서 분리된 각각의 문자열에 순차적으로 접근하면서 반복을 수행
    
    [표현법]
    &lt;c:forTokens var="각값을보관할변수" items="분리시키고자하는원본문자열" delims="구분자"&gt;
          반복적으로 수행할 내용물들 
     &lt;/c:forTokens&gt;
   </pre>
   ...
   [ 표현법 ]
   &lt;c:url var="만들어진url을담을변수명" value="요청할 url"&gt;
   ...
   <!-- 기존 방식 -->
   <a href="list.do?currentPage=1&num2">기존방식</a>
   
   <!--core library를 이용한 방식 -->
   <c:url var="query" value="list.do">
     <c:param name="currentPage" value="1" />
     <c:param name="num" value="2" />
   </c:url> <!-- 내부적으로 list.do?currentPage=1&num2가 만들어져서 query라는 변수에 담겨있음 -->
